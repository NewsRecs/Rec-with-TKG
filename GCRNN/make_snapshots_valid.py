"""
snapshot에 존재하는 노드들만 생성하여 메모리 사용 대폭 감소
"""
...
# news_embeddings = torch.randn(len(all_news_ids), 128, device=device)


global snapshots_user_ids
global snapshots_news_ids
global snapshots_cats_ids
snapshots_user_ids = []
snapshots_news_ids = []
snapshots_cats_ids = []
# 4) Main 함수
def main():
    ...
    for period_start, group in tqdm(grouped, desc="Making snapshots"):
        ...
        """
        edges_src_snapshot, edges_dst_snapshot, edge_cats_snapshot
        : 각각 전체 유저, 뉴스, 카테고리에 대하여, snapshot에 존재하는 유저, 뉴스, 카테고리의 indices
        """
        edges_src_snapshot = group['history_user'].map(user2int).values
        # edges_src = [i for i, u in enumerate(edges_src_snapshot)]
        edges_dst_snapshot = group['clicked_news'].map(news2int).values
        # edges_dst = [i for i, n in enumerate(edges_dst_snapshot)]
        ...
        # 로컬 인덱스(0, ..., N-1) 매핑
        local_user2int = {uid: i for i, uid in enumerate(snapshot_user_ids)}
        local_news2int = {nid: i for i, nid in enumerate(snapshot_news_ids)}
        
        # 엣지(src->dst) 배열 생성 (로컬 인덱스로 변환)
        edges_src = group['history_user'].apply(lambda x: local_user2int[x]).values
        edges_dst = group['clicked_news'].apply(lambda x: local_news2int[x]).values
        
        # 4.2.2) 그래프 생성: 전체 유저/뉴스 노드 수 지정
        g = dgl.heterograph(
            {('user', 'clicked', 'news'): (edges_src, edges_dst),
             ('news', 'clicked_reverse', 'user'): (edges_dst, edges_src)},   # 이후 유저 노드 GCN 연산을 위해 역방향 edges 추가
            num_nodes_dict={'user': len(snapshot_user_ids), 'news': len(snapshot_news_ids)}
        ).to(device)

        # 4.2.3) 노드 (유저, 뉴스) 임베딩 할당
        snapshot_user_indices = [user2int[u] for u in snapshot_user_ids]
        snapshot_news_indices = [news2int[u] for u in snapshot_news_ids]
        
        snapshot_user_embeddings = user_embeddings[snapshot_user_indices]
        snapshot_news_embeddings = news_embeddings[snapshot_news_indices]
        
        g.nodes['user'].data['feat'] = snapshot_user_embeddings
        g.nodes['news'].data['feat'] = snapshot_news_embeddings

        # 4.2.4) 엣지 (카테고리) 임베딩 할당
        edge_cats = group['category'].map(cat2idx).values
        ...
        snapshots.append(g)
        snapshots_user_ids.append(edges_src_snapshot)
        snapshots_news_ids.append(edges_dst_snapshot)
        snapshots_cats_ids.append(edge_cats_snapshot)
        
    print(f"Total snapshots: {len(snapshots)}")

    # 스냅샷 저장
    snapshot_save_path = "./Adressa_4w/history/snapshots.pkl"
    with open(snapshot_save_path, 'wb') as f:
        pickle.dump(snapshots, f)

    print(f"Snapshots saved to: {snapshot_save_path}")
...
# 생성된 snapshot 정보 불러오는 함수
def extract_and_save_embeddings(snapshots_path, embeddings_save_path):
    """
    snapshots.pkl 파일에서 각 스냅샷의 유저/뉴스 노드 임베딩과 엣지 임베딩을 추출하여 저장합니다.
    
    Args:
        snapshots_path (str): snapshots.pkl 파일의 경로.
        embeddings_save_path (str): 추출된 임베딩을 저장할 파일 경로.
    """
    # snapshots.pkl 로드
    ...
    edge_embeddings_list2 = []
    
    # 각 스냅샷 처리
    for idx, g in enumerate(tqdm(snapshots, desc="Processing snapshots")):
        # 유저 노드 임베딩 추출 
        user_feats = g.nodes['user'].data['feat']
        user_embeddings_list.append(user_feats)
        
        # 뉴스 노드 임베딩 추출
        news_feats = g.nodes['news'].data['feat']
        news_embeddings_list.append(news_feats)
        
        # 엣지 임베딩 추출
        edge_feats = g.edges['clicked'].data['feat']
        edge_embeddings_list.append(edge_feats)
        
        edge_feats2 = g.edges['clicked_reverse'].data['feat']
        edge_embeddings_list2.append(edge_feats2)
    
    # 임베딩 저장
    embeddings = {
        'user_embeddings': user_embeddings_list,
        'news_embeddings': news_embeddings_list,
        'clicked_edge_embeddings': edge_embeddings_list,
        'clicked_reverse_edge_embeddings': edge_embeddings_list2,
        'snapshot_user_ids': snapshots_user_ids,
        'snapshot_news_ids': snapshots_news_ids,
        'snapshot_cats_ids': snapshots_cats_ids
    }
    
    torch.save(embeddings, embeddings_save_path)
    print(f"임베딩이 '{embeddings_save_path}'에 저장되었습니다.")
    

if __name__ == "__main__":
    main()
    snapshots_path = './Adressa_4w/history/snapshots.pkl'        # snapshots.pkl 파일 경로
    embeddings_save_path = './Adressa_4w/history/snapshots_embeddings.pkl' # 임베딩 저장 파일 경로
    extract_and_save_embeddings(snapshots_path, embeddings_save_path)
