...
def main():
    ...
    g, splitted_g = split_train_graph(
        snapshot_weeks,
        interval_hours, 
        f'psj/Adressa_{week}w/datas/total_graph_full_reciprocal_{interval_minutes}m.bin'
    )
    # print(g.number_of_nodes())
    # exit()
    # with open('./psj/Adressa_4w/train/train_datas.pkl', 'rb') as f:
    #     datas = pickle.load(f)
    datas = make_train_datas(interval_minutes=interval_minutes, week=week)
    train_news, train_category, train_time = zip(*datas)


    # 사전 학습된 단어 로드
    word2int = pd.read_csv(os.path.join(f'psj/Adressa_{week}w/history/', 'word2int.tsv'), sep='\t')
    word_to_idx = word2int.set_index('word')['int'].to_dict()
    embedding_file_path = os.path.join(f'psj/Adressa_{week}w/history/', 'pretrained_word_embedding.npy')
    ...
    combined_news_df = pd.read_csv(
        f'psj/Adressa_{week}w/history/all_news.tsv',   # _nyheter_splitted
        sep='\t'
    ).rename(columns={'newsId': 'clicked_news'})
    
    all_news_ids = pd.read_csv(f'psj/Adressa_{week}w/history/news2int.tsv', sep='\t')['news_id']
    news_num = len(all_news_ids)
    user2int_df = pd.read_csv(os.path.join(f'psj/Adressa_{week}w/history/', 'user2int.tsv'), sep='\t')
    ...
    category2int = pd.read_csv(f'psj/Adressa_{week}w/datas/category2int.tsv', sep='\t')   # _nyheter_splitted_for_NE    
    ...
    train_ns_idx_batch, _ = ns_indexing(f'psj/Adressa_{week}w/train/train_ns.tsv', original_batch_size, user_num=user_num)
    ...
    test_ns_idx_batch, test_cand_score_weight_batch = ns_indexing(f'psj/Adressa_{week}w/test/test_ns.tsv', original_batch_size, user_num=user_num, test=True)
    ...
    early_stopper = EarlyStopping(
        ...
        min_delta=1e-4,
        ckpt_dir=f'psj/Adressa_{week}w/1_hop_ckpt/bs_{original_batch_size}_lr_{learning_rate}_seed_{random_seed}', 
        verbose=True,
